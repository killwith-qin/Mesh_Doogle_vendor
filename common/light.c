...
void light_transition_proc()
{
	...
	foreach_arr(i,light_res_sw)
	{
		...
		foreach_arr(trans_type,light_res_sw[i].trans)
		{
			...
			if(p_trans->delay_ms) //exist delay time, ie.not immediate.  be use to judge to reach target state.
			{
				p_trans->delay_ms--;
				if((0 == p_trans->delay_ms) && (0 == p_trans->remain_t_ms))  //delay_ms and total time is expired.
				{
					complete_level = 1;
				}else
				{
			...
			{
				if(p_trans->remain_t_ms) //not reach target state, BUT reach delay time state.
				{	
					if(p_trans->present != p_trans->target)
					{
						if(0 == (p_trans->remain_t_ms % LIGHT_ADJUST_INTERVAL))  //delay time reach state.BUT not target state.
						{
                            s16 next_val = light_get_next_level(i, trans_type);
                            st_pub_list_t pub_list = {{0}};
                            pub_list.no_dim_refresh_flag = 1;
                            dim_refresh_flag = 1;
							light_g_level_set_idx(i, next_val, 0, trans_type, &pub_list);// no_dim_refresh_flag = 1;BUT flash new state
			...
			if(complete_level)
			{
                ...
                pub_list.no_dim_refresh_flag = 1;
                dim_refresh_flag = 1;
				light_g_level_set_idx(i, p_trans->target, 0, trans_type, &pub_list); // target_state,no_dim_refresh_flag = 1;BUT flash new state
				light_transition_log(trans_type, p_trans->present);
		...
		if(dim_refresh_flag)
		{
            light_dim_refresh(i);   // dim refresh only when all level set ok, only in here really set PWM.
		}
	}

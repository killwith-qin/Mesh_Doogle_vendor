...
void cb_User_ADV_Hander(u32 h, u8 *p, int n)
{


	event_adv_report_t *pa = (event_adv_report_t *)p;

	LOG_USER_MSG_INFO((u8 *)pa, sizeof(event_adv_report_t),"Receive_ADV_Message: ", 0);

}

const unsigned char GW_Mesh_GW_MacID[6] = {0x20,0x19,0x11,0x22,0xFF,0x11};
...
int app_event_handler (u32 h, u8 *p, int n)
{
	...
	//qinwei print ADV Packet.

	//LOG_USER_MSG_INFO(p, sizeof(event_adv_report_t),"Receive_ADV_Message: ", 0);
	...
	//------------ ADV packet --------------------------------------------
		if (subcode == HCI_SUB_EVT_LE_ADVERTISING_REPORT)	// ADV packet
		{
			...
			//qinwei print ADV Packet.
            if((memcmp(pa->mac,GW_Mesh_GW_MacID,6)) == 0)
            {
                LOG_USER_MSG_INFO(p, sizeof(event_adv_report_t),"Receive_ADV_Message: ", 0);
            }
